...
def test__main__download_files__when_no_existing_local_files(mockito_handler):
...
def test__main__download_files__with_one_existing_local_file(mockito_handler):
...
def test__main__download_files__with_all_files_locally_exist(mockito_handler):
...
def test__main__upload_files__when_some_existing_remote_files(mockito_handler):
    existing_local_files = ['squirrel.jpg', 'bowtie.png', 'octoparty.gif', 'flow.png']
    when(emoji_bulk_migrator)._get_existing_local_files("path/to/files").thenReturn(existing_local_files)
    when(api_handler).get_emoji_list().thenReturn(existing_remote_files)
    expect(api_handler).load_emoji(...)
...
def test__main__upload_files__when_all_existing_remote_files(mockito_handler):
    existing_local_files = ['squirrel.jpg', 'bowtie.png', 'octoparty.gif', 'flow.png']
    existing_remote_files = {
        "ok": "true",
        "emoji": {
